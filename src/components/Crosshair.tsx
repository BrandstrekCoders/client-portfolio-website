...
const Crosshair = ({ color = 'white', containerRef }: CrosshairProps) => {
    ...
    useEffect(() => {
        ...
        // We need to keep reference to the requestanimationframe to cancel it
        let rafId: number;
        ...
        // We need to apply this to links inside the container
        const links = containerRef?.current ? containerRef.current.querySelectorAll('a, button, .cursor-pointer') : document.querySelectorAll('a, button, .cursor-pointer');
    ...
    return (
        ...
        >
            {/* We need svg definitions for the filters */}
            <svg style={{ position: 'absolute', left: 0, top: 0, width: '100%', height: '100%', opacity: 0 }}>
                <defs>
